...
void ExampleEventHandlerUnsubscribe()
{
    ...
    // Subscriber objects can be linked to subscriptions. When the subscriber object is dealocated
    // it automaticly unsubscribe from the linked events. Subscriber objects also have an UnsubscribeAll()
    // function that can be used to manually unsubscribe from events linked to it.

    // Create a event handler
    VLS::Event::EventHandler<> valueChanged;

    // Option 1: It is not possible to unsubscribe with subscribe Persistent
    // This is the easiest option if the event reciever lives at least as long as the event handler. 
    ...
    // This is the easiest option when having one subscriber object pr. subscription.
    VLS::Event::Subscriber::UPtr returnSubscriber = valueChanged.Subscribe([]() { 
        cout << "> Return Subscriber  Event" << endl; 
    });

    // Option 3: Provide a subscriber object that gets linked with the subscription.
    // This is the easiest option when having one subscriber for multiple subscriptions.
