...
// default but can be changed with this flag
//#define VLS_EVENT_ONE_SUBSCRIPTION_PER_SUBSCRIBER

namespace VLS::Event {

/// <summary>
/// Handles events by providing the possibility to subscribe to and trigger an
/// event, with arguments matching the variadic temple of the class.
/// </summary>
/// <remarks>
/// Subscriptions can be persistent(no unsubscribe option) or be handled by a
/// subscriber. If an reference to a subscriber object is provided it will be
/// used, otherwise a unique point to a subscriber object will be returned. The
/// subscriber provides functions that can be used to unsubscribe to the event.
/// The subscriber will unsubscribe any existing subscription automatically if
/// deallocated.
///
...
/// the supplied function directly, it will be packed into a void(void) lambda
/// function with all the function arguments and enqueued on the event loop.
///
/// Value, const reference and pointer argument types are supported. Reference
/// arguments are not. Const reference and value arguments types needs a copy
/// constructor. If a class with no copy constructor needs to be return in an
/// event callback a pointer can be used.
///
/// Warning: Proper care should be taking with function argument to make sure
/// they are still in scope when the event loop gets around to calling the
/// enqueued function.
/// </remarks>
...
class EventHandler : public Publisher, public IEventHandler<Types...> {
 public:
  ...
  Subscriber::UPtr Subscribe(const std::function<void(Types...)>& func,
                             IEventLoop* eventLoop = nullptr) override {
  ...
  bool Subscribe(Subscriber& subscriber,
    ...
    if (Publisher::Subscribe(subscriber)) {
      m_functionList.push_back(
          std::make_unique<EventData<Types...>>(&subscriber, func, eventLoop));
      return true;
    }
    return false;
#else
  ...
  void SubscribePersistent(const std::function<void(Types...)>& func,
                           IEventLoop* eventLoop = nullptr) override {
    std::lock_guard<std::mutex> guard(m_mutex);
    this->m_functionList.push_back(
        std::make_unique<EventData<Types...>>(nullptr, func, eventLoop));
  }
  ...
  virtual void Trigger(Types... args) {
    // Subscriber data is copied to avoid having the mutex locked when event
    // functions are called. This will prevent a potential deadlocked if
    // subscribers unsubscribe or triggers the same event during event triggered
    // code.
    std::vector<std::pair<std::function<void(Types...)>, IEventLoop*>>
        functionList;
    m_mutex.lock();
    ...
    for (auto data : functionList) {
      if (data.second == nullptr) {
        ...
        if constexpr (sizeof...(args) == 0) {
          // The function is already a callable void(void) function so it is
          // just added to the event loop.
          data.second->Enqueue(data.first);
        } else {
          // Encapsulates the function call in a void(void) lambda and queue it
          // on the event loop.
          data.second->Enqueue([data, args...]() { data.first(args...); });
        }
      }
    }
  }
 ...
 protected:
  ...
  /// </summary>
  /// <remarks>
  /// This function will be triggered by the publisher base after a subscriber
  /// has unsubscribed.
  /// </remarks>
