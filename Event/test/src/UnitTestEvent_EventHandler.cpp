...
void SetEventValue1(int v1) {
    sEventValue = v1;
}

void SetEventValue2(int v1, int v2) {
    sEventValue = v1 + v2;
}
...
TEST(VLSEventHandler0, SuccessMember) {
    Event::EventHandler<> eventHandler;
    EventRetriever cr;
    auto u = eventHandler.Subscribe(VLS::Event::Func::Bind(&cr, &EventRetriever::SetValue));
...
TEST(VLSEventHandler0, ScopeUnsubscribe1) {
    ...
    int value = 0;
    {
        // The Unsubscriber is not kept alive so it unsubscribes before the eventHandler is triggered
        auto u = eventHandler.Subscribe([&value] { value = 42; });
    }
    eventHandler.Trigger();
    EXPECT_EQ(value, 0);
...
TEST(VLSEventHandler0, EventLoop) {
    ...
    int value = 0;
    EXPECT_TRUE(eventHandler.Subscribe(sub, [&value, &threadName]() {
        value = 42;
        threadName = VLS::Event::EventLoop::ThreadName();
        }, &eventLoop));
    EXPECT_EQ(value, 0);
    eventHandler.Trigger();
    eventLoop.Stop();
...
TEST(VLSEventHandler1, SuccessMember) {
    Event::EventHandler<int> eventHandler;
    EventRetriever cr;
    auto u = eventHandler.Subscribe(Event::Func::Bind(&cr, &EventRetriever::SetValue1));
    EXPECT_EQ(cr.value, 0);
    eventHandler.Trigger(42);
    EXPECT_EQ(cr.value, 42);
}
...
TEST(VLSEventHandler2, SuccessLamda) {
    ...
    int value = 0;
    auto u = eventHandler.Subscribe([&value](int v1, int v2) { value = v1 + v2; });
    EXPECT_EQ(value, 0);
...
TEST(VLSEventHandler2, SuccessMember) {
    Event::EventHandler<int, int> eventHandler;
    EventRetriever cr;
    auto u = eventHandler.Subscribe(Event::Func::Bind(&cr, &EventRetriever::SetValue2));
    EXPECT_EQ(cr.value, 0);
...
TEST(VLSEventHandler2, Persistent) {
    Event::EventHandler<int, int> eventHandler;
    int value = 0;
    eventHandler.SubscribePersistent([&value](int v1, int v2) { value = v1 + v2; });
    EXPECT_EQ(value, 0);
    eventHandler.Trigger(20, 22);
    EXPECT_EQ(value, 42);
}
...
TEST(VLSEventHandler2, SuppliedSubsriber) {
    ...
    Event::Subscriber sub;
    int value = 0;
    EXPECT_TRUE(eventHandler.Subscribe(sub, [&value](int v1, int v2) { value = v1 + v2; }));
    EXPECT_EQ(value, 0);
    eventHandler.Trigger(20, 22);
    EXPECT_EQ(value, 42);
    sub.UnsubscribeAll();
...
TEST(VLSEventHandler2, EventLoop) {
    ...
    std::string threadName = VLS::Event::EventLoop::ThreadName();
    int value = 0;
    EXPECT_TRUE(eventHandler.Subscribe(sub, [&value, &threadName](int v1, int v2) { 
        value = v1 + v2;
        threadName = VLS::Event::EventLoop::ThreadName();
    }, &eventLoop));
    EXPECT_EQ(value, 0);
...
TEST(VLSEventHandler2, MultipeSubscribers) {
    ...
    auto sub2 = eventHandler.Subscribe([&value2](int v1, int v2) { value2 = v1 - v2; });
    eventHandler.SubscribePersistent([&value3](int v1, int v2) { value3 = v1 * v2; });
...
TEST(VLSEventHandler7, Success1) {
    Event::EventHandler<int, unsigned int, double, unsigned short, size_t, long, char> eventHandler;
    int value = 0;
    auto u = eventHandler.Subscribe([&value](
        int v1, 
        unsigned int v2, 
        double v3, 
        unsigned short v4, 
